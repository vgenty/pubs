...
case `whoami` in
    (uboonepro)
        echo uboonepro account should use the other setup script.
	echo stay away.
	echo Exiting with exterme prejudice.
	return 1
	;;
    (*)
        echo Setting up PUBS for $USER account ...
	;;
esac
...
#	export X509_USER_PROXY=/home/uboonepro/uboonepro_production_near1_proxy_file;
	...
	source /home/uboonepro/.sql_access_test/uboonepro_test_conf.sh;
	...
	case `uname -n` in
		...
		setup postgresql v9_3_6 -q p279;
	        setup uboonedaq_datatypes v6_15_03c -q e7:debug;
		...
		#let it be setup as a dependency of the uboonecode version
	        setup uboonedaq_datatypes v6_15_03c -q e7:debug;
		setup uboonecode v04_26_02 -q prof:e7;
		setup pyqtgraph;
